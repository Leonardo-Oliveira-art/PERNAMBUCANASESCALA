...
function render(){ persist(); renderTabs(); renderPanel(); renderPrint(); renderHeaderLogo(); }
...
function sheetHTML(printTarget){
  ...
  const block = sec => {
    ...
    let body = rows.map(r=>`<tr><td>${esc(r.name)}</td><td>${esc(r.time)}</td></tr>`).join("");
    for(let i=rows.length;i<MIN_ROWS;i++) body += "<tr><td>&nbsp;</td><td>&nbsp;</td></tr>";
    return `<div class="sheet-block">
      ...
      <table class="sheet-table"><thead><tr><th>COLABORADOR(A)</th><th>HORÁRIO</th></tr></thead><tbody>${body}</tbody></table>
    </div>`;
  };
...
function builderHTML(){
  const sectors = settings.sectors.map(sec=>{
    ...
    const items = list.length ? list.map((it,i)=>`
      <div class="assign">
        <div class="info"><b>${esc(it.name)}</b><small>${esc(it.time||"sem horário")}</small></div>
...
function openAssign(sectorId, itemId){
  ...
  dlgState = { sectorId, id:item?item.id:uid(), editing:!!item, name:item?item.name:"", time:item?item.time:"" };
...
function saveAssign(){
  ...
  const item = { id:st.id, name, time:(st.time||"").trim() };
